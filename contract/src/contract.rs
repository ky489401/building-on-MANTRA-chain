...
pub fn execute_update_entry(
    ...
    priority: Option<Priority>,
    owner: String,  // ✅ Accept owner argument
) -> Result<Response, ContractError> {
    LIST.update(deps.storage, id, |entry| {
        match entry {
            Some(mut existing_entry) => {
                if existing_entry.owner != owner {
                ...
                if let Some(stat) = status {
                    existing_entry.status = stat;
                }
                if let Some(prio) = priority {
                    existing_entry.priority = prio;
                }
                Ok(existing_entry)
            }
            None => Err(ContractError::CustomError {
                val: "Task not found".to_string(),
            }),
        }
    })?;
...
pub fn execute_delete_entry(
    ...
    let task = LIST.may_load(deps.storage, id)?;
